...
const Size = ({ data, setSizePrice, item, setRecommendedSizePrice }) => {
  ...
  useEffect(() => {
    if (item == '100') {
      sessionStorage.setItem(
        `recommendedSizePrice${data.product_name_kor}`,
        recommendedSizeCoast,
      )
      sessionStorage.setItem(
        `recommendedSize${data.product_name_kor}`,
        recommendedSize,
      )
    } else {
  ...
  return (
    <div className="mx-auto mt-5 block max-w-[420px] flex-grow lg:mx-0 lg:mr-6 lg:ml-1">
